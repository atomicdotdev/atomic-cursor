...
Then run `atomic vault sync` to persist your edits into the vault database. The intent file lives on disk under `.vault/`, but `atomic vault intent show`/`update` read from the **database**, not the file — without `sync` they see the original placeholder template, and `update` re-materializes the database copy over the file, clobbering your edits. `atomic vault sync` is NOT `atomic record`: it only moves your `.vault/` edits into the vault database, and you must run it yourself even though Cursor hooks handle recording.

### 2b. Simplification guard

Before you finalize the intent, audit every choice that is *simpler than* or *diverges from* a reference — the standard library, an existing implementation, a spec, or a prior version. The simpler choice almost always **drops a behavior the reference guaranteed**, and that's where silent correctness gaps hide.

For each such decision, name the behavior the simpler choice drops — interrupted/partial operations, error or panic states, round-trip fidelity, ordering, resource cleanup, overflow/empty/boundary inputs — and for each dropped behavior either **pin it** as an acceptance criterion, **record it as out-of-scope** with the consequence stated, or **ask the user**. Never leave it unstated. A decision about API *shape* is not a decision about *behavior*: the same signature can be implemented correctly or incorrectly.
...
**Do NOT run `atomic add` or `atomic record`.** The hook system records your changes automatically with full AI provenance (model, tokens, session, timing) when the turn ends. (`atomic vault sync` is not `atomic record` — it only moves your `.vault/` edits into the vault database, and you must run it.)
...
- **Simplification guard.** When you choose an approach simpler than or divergent from a reference (the standard library, an existing implementation, a spec), name the behavior the simpler choice drops (interrupted/partial operations, error states, round-trip fidelity, boundaries) and either pin it as an acceptance criterion, record it as out-of-scope with the consequence stated, or ask the user — never leave it unstated. A decision about API shape is not a decision about behavior.
